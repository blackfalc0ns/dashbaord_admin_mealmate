...
```mermaid
flowchart TD
    Start(("Start F097 Commission Eligibility Rules"))
    S1["ربط المؤثر/الكود."]
...
sequenceDiagram
    ...
    User->>UI: Open Commission Eligibility Rules
    UI->>API: Request /api/v1/commission_eligibility_rules
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F097 data
...
classDiagram
    ...
    User --> F097_Entity : creates/updates
    F097_Entity --> AuditLog : records
    F097_Entity --> NotificationLog : triggers
```

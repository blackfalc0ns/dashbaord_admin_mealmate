...
```mermaid
flowchart TD
    Start(("Start F100 Influencer Withdrawal Request"))
    S1["ربط المؤثر/الكود."]
...
sequenceDiagram
    ...
    User->>UI: Open Influencer Withdrawal Request
    UI->>API: Request /api/v1/influencer_withdrawal_request
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F100 data
...
classDiagram
    ...
    User --> F100_Entity : creates/updates
    F100_Entity --> AuditLog : records
    F100_Entity --> NotificationLog : triggers
```

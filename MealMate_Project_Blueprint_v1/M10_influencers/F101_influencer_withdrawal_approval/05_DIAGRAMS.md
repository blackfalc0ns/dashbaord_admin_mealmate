...
```mermaid
flowchart TD
    Start(("Start F101 Influencer Withdrawal Approval"))
    S1["ربط المؤثر/الكود."]
...
sequenceDiagram
    ...
    User->>UI: Open Influencer Withdrawal Approval
    UI->>API: Request /api/v1/influencer_withdrawal_approval
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F101 data
...
classDiagram
    ...
    User --> F101_Entity : creates/updates
    F101_Entity --> AuditLog : records
    F101_Entity --> NotificationLog : triggers
```

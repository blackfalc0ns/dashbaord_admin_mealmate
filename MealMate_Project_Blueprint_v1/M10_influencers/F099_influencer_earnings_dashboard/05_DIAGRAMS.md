...
```mermaid
flowchart TD
    Start(("Start F099 Influencer Earnings Dashboard"))
    S1["ربط المؤثر/الكود."]
...
sequenceDiagram
    ...
    User->>UI: Open Influencer Earnings Dashboard
    UI->>API: Request /api/v1/influencer_earnings_dashboard
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F099 data
...
classDiagram
    ...
    User --> F099_Entity : creates/updates
    F099_Entity --> AuditLog : records
    F099_Entity --> NotificationLog : triggers
```

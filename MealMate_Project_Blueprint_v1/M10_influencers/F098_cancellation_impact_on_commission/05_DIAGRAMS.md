...
```mermaid
flowchart TD
    Start(("Start F098 Cancellation Impact on Commission"))
    S1["ربط المؤثر/الكود."]
...
sequenceDiagram
    ...
    User->>UI: Open Cancellation Impact on Commission
    UI->>API: Request /api/v1/cancellation_impact_on_commission
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F098 data
...
classDiagram
    ...
    User --> F098_Entity : creates/updates
    F098_Entity --> AuditLog : records
    F098_Entity --> NotificationLog : triggers
```

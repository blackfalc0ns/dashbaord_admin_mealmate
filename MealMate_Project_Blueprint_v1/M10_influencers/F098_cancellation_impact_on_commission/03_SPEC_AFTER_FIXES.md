# F098 — Spec After Fixes

## Purpose
تنفيذ **Cancellation Impact on Commission** كميزة جاهزة للتطوير بعد معالجة نقاط الضعف والفجوات.
...
|---|---|---|
| GET | `/api/v1/cancellation_impact_on_commission` | List/search |
| GET | `/api/v1/cancellation_impact_on_commission/{id}` | Details |
| POST | `/api/v1/cancellation_impact_on_commission` | Create/request |
| PUT | `/api/v1/cancellation_impact_on_commission/{id}` | Update |
| POST | `/api/v1/cancellation_impact_on_commission/{id}/actions/{action}` | State transition |

...
```mermaid
flowchart TD
    Start(("Start F013 Customer Subscription Pricing"))
...
sequenceDiagram
    ...
    User->>UI: Open Customer Subscription Pricing
    UI->>API: Request /api/v1/customer_subscription_pricing
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F013 data
...
classDiagram
    ...
    User --> F013_Entity : creates/updates
    F013_Entity --> AuditLog : records
    F013_Entity --> NotificationLog : triggers
```

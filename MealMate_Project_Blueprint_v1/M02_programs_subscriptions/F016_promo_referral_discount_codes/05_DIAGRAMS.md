...
```mermaid
flowchart TD
    Start(("Start F016 Promo, Referral & Discount Codes"))
...
sequenceDiagram
    ...
    User->>UI: Open Promo, Referral & Discount Codes
    UI->>API: Request /api/v1/promo_referral_discount_codes
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F016 data
...
classDiagram
    ...
    User --> F016_Entity : creates/updates
    F016_Entity --> AuditLog : records
    F016_Entity --> NotificationLog : triggers
```

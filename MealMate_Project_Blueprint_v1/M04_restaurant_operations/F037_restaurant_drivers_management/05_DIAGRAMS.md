...
```mermaid
flowchart TD
    Start(("Start F037 Restaurant Drivers Management"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Drivers Management
    UI->>API: Request /api/v1/restaurant_drivers_management
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F037 data
...
classDiagram
    ...
    User --> F037_Entity : creates/updates
    F037_Entity --> AuditLog : records
    F037_Entity --> NotificationLog : triggers
```

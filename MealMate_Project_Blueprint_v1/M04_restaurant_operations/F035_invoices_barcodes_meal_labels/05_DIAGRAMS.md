...
```mermaid
flowchart TD
    Start(("Start F035 Invoices, Barcodes & Meal Labels"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Invoices, Barcodes & Meal Labels
    UI->>API: Request /api/v1/invoices_barcodes_meal_labels
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F035 data
...
classDiagram
    ...
    User --> F035_Entity : creates/updates
    F035_Entity --> AuditLog : records
    F035_Entity --> NotificationLog : triggers
```

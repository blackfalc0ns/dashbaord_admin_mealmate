...
```mermaid
flowchart TD
    Start(("Start F126 Profitability Alerts"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Profitability Alerts
    UI->>API: Request /api/v1/profitability_alerts
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F126 data
...
classDiagram
    ...
    User --> F126_Entity : creates/updates
    F126_Entity --> AuditLog : records
    F126_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F131 Notifications Center"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Notifications Center
    UI->>API: Request /api/v1/notifications_center
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F131 data
...
classDiagram
    ...
    User --> F131_Entity : creates/updates
    F131_Entity --> AuditLog : records
    F131_Entity --> NotificationLog : triggers
```

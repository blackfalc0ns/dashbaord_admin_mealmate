...
```mermaid
flowchart TD
    Start(("Start F125 Finance Dashboard"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Finance Dashboard
    UI->>API: Request /api/v1/finance_dashboard
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F125 data
...
classDiagram
    ...
    User --> F125_Entity : creates/updates
    F125_Entity --> AuditLog : records
    F125_Entity --> NotificationLog : triggers
```

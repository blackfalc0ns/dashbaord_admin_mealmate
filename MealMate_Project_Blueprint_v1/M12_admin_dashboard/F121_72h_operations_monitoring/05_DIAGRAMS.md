...
```mermaid
flowchart TD
    Start(("Start F121 72h Operations Monitoring"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open 72h Operations Monitoring
    UI->>API: Request /api/v1/72h_operations_monitoring
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F121 data
...
classDiagram
    ...
    User --> F121_Entity : creates/updates
    F121_Entity --> AuditLog : records
    F121_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F119 Pending Approvals"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Pending Approvals
    UI->>API: Request /api/v1/pending_approvals
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F119 data
...
classDiagram
    ...
    User --> F119_Entity : creates/updates
    F119_Entity --> AuditLog : records
    F119_Entity --> NotificationLog : triggers
```

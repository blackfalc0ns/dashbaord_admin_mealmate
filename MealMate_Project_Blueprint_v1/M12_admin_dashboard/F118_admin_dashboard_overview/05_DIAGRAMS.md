...
```mermaid
flowchart TD
    Start(("Start F118 Admin Dashboard Overview"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Admin Dashboard Overview
    UI->>API: Request /api/v1/admin_dashboard_overview
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F118 data
...
classDiagram
    ...
    User --> F118_Entity : creates/updates
    F118_Entity --> AuditLog : records
    F118_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F120 Operations Monitoring"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Operations Monitoring
    UI->>API: Request /api/v1/operations_monitoring
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F120 data
...
classDiagram
    ...
    User --> F120_Entity : creates/updates
    F120_Entity --> AuditLog : records
    F120_Entity --> NotificationLog : triggers
```

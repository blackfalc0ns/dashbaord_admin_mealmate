...
```mermaid
flowchart TD
    Start(("Start F129 Roles & Permissions Administration"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Roles & Permissions Administration
    UI->>API: Request /api/v1/roles_permissions_administration
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F129 data
...
classDiagram
    ...
    User --> F129_Entity : creates/updates
    F129_Entity --> AuditLog : records
    F129_Entity --> NotificationLog : triggers
```

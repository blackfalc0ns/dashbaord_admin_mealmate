...
```mermaid
flowchart TD
    Start(("Start F022 72-Hour Lock Rule"))
    S1["فتح التقويم."]
...
sequenceDiagram
    ...
    User->>UI: Open 72-Hour Lock Rule
    UI->>API: Request /api/v1/72_hour_lock_rule
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F022 data
...
classDiagram
    ...
    User --> F022_Entity : creates/updates
    F022_Entity --> AuditLog : records
    F022_Entity --> NotificationLog : triggers
```

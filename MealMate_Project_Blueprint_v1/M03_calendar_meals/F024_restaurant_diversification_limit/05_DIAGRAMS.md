...
```mermaid
flowchart TD
    Start(("Start F024 Restaurant Diversification Limit"))
    S1["فتح التقويم."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Diversification Limit
    UI->>API: Request /api/v1/restaurant_diversification_limit
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F024 data
...
classDiagram
    ...
    User --> F024_Entity : creates/updates
    F024_Entity --> AuditLog : records
    F024_Entity --> NotificationLog : triggers
```

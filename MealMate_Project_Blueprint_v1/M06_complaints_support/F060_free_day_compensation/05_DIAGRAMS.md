...
```mermaid
flowchart TD
    Start(("Start F060 Free Day Compensation"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Free Day Compensation
    UI->>API: Request /api/v1/free_day_compensation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F060 data
...
classDiagram
    ...
    User --> F060_Entity : creates/updates
    F060_Entity --> AuditLog : records
    F060_Entity --> NotificationLog : triggers
```

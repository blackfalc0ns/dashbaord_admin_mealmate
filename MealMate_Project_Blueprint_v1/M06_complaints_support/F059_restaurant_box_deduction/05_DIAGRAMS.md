...
```mermaid
flowchart TD
    Start(("Start F059 Restaurant Box Deduction"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Box Deduction
    UI->>API: Request /api/v1/restaurant_box_deduction
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F059 data
...
classDiagram
    ...
    User --> F059_Entity : creates/updates
    F059_Entity --> AuditLog : records
    F059_Entity --> NotificationLog : triggers
```

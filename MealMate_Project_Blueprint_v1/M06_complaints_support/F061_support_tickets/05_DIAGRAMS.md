...
```mermaid
flowchart TD
    Start(("Start F061 Support Tickets"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Support Tickets
    UI->>API: Request /api/v1/support_tickets
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F061 data
...
classDiagram
    ...
    User --> F061_Entity : creates/updates
    F061_Entity --> AuditLog : records
    F061_Entity --> NotificationLog : triggers
```

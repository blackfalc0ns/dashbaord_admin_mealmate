...
```mermaid
flowchart TD
    Start(("Start F056 Restaurant Complaint Response"))
    S1["إنشاء الحالة."]
    S2["فحص الطلب والأدلة."]
    S3["التصنيف والإسناد."]
    S4["جمع رد المطعم."]
    S5["قرار موثق."]
    S6["تعويض/خصم بعد الموافقة."]
    Done(("Done"))
    Start --> S1
    S1 --> S2
    S2 --> S3
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Complaint Response
    UI->>API: Request /api/v1/restaurant_complaint_response
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F056 data
...
classDiagram
    ...
    User --> F056_Entity : creates/updates
    F056_Entity --> AuditLog : records
    F056_Entity --> NotificationLog : triggers
```

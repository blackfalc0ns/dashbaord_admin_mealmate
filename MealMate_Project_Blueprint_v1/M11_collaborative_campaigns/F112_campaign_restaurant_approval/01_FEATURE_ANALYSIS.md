...
الميزة تمثل بوابة قبول كيان جديد أو اعتماده، لذلك ضعف معايير القبول يؤدي إلى قرارات غير موحدة ومخاطر تشغيلية.
...
تحتاج state machine تفصل بين المسودة، الرفع، المراجعة، الاستكمال، الرفض، الاعتماد، والتفعيل.
...
- يلزم حفظ reviewer, reason, decision timestamp, document version, وbefore/after لكل قرار.
...
2. **تفصيل API Contract**
   - إضافة schemas لكل endpoint:
     - `GET /api/v1/campaign_restaurant_approval`
     - `GET /api/v1/campaign_restaurant_approval/{id}`
     - `POST /api/v1/campaign_restaurant_approval`
     - `PUT /api/v1/campaign_restaurant_approval/{id}`
     - `POST /api/v1/campaign_restaurant_approval/{id}/actions/{action}`
...
3. **تفصيل صلاحيات Role/Scope**
   - ربط كل شاشة وكل endpoint بصلاحية مثل `F112.View`, `F112.Create`, `F112.Update`, `F112.Override`.

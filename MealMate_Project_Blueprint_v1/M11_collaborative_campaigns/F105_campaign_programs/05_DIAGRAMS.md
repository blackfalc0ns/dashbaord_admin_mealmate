...
```mermaid
flowchart TD
    Start(("Start F105 Campaign Programs"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Programs
    UI->>API: Request /api/v1/campaign_programs
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F105 data
...
classDiagram
    ...
    User --> F105_Entity : creates/updates
    F105_Entity --> AuditLog : records
    F105_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F110 Campaign Subscriber Cap"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Subscriber Cap
    UI->>API: Request /api/v1/campaign_subscriber_cap
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F110 data
...
classDiagram
    ...
    User --> F110_Entity : creates/updates
    F110_Entity --> AuditLog : records
    F110_Entity --> NotificationLog : triggers
```

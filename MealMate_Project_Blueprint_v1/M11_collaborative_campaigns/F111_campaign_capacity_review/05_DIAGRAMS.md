...
```mermaid
flowchart TD
    Start(("Start F111 Campaign Capacity Review"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Capacity Review
    UI->>API: Request /api/v1/campaign_capacity_review
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F111 data
...
classDiagram
    ...
    User --> F111_Entity : creates/updates
    F111_Entity --> AuditLog : records
    F111_Entity --> NotificationLog : triggers
```

...
2. **تفصيل API Contract**
   - إضافة schemas لكل endpoint:
     - `GET /api/v1/campaign_capacity_review`
     - `GET /api/v1/campaign_capacity_review/{id}`
     - `POST /api/v1/campaign_capacity_review`
     - `PUT /api/v1/campaign_capacity_review/{id}`
     - `POST /api/v1/campaign_capacity_review/{id}/actions/{action}`
...
3. **تفصيل صلاحيات Role/Scope**
   - ربط كل شاشة وكل endpoint بصلاحية مثل `F111.View`, `F111.Create`, `F111.Update`, `F111.Override`.

...
```mermaid
flowchart TD
    Start(("Start F114 Campaign Pause"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Pause
    UI->>API: Request /api/v1/campaign_pause
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F114 data
...
classDiagram
    ...
    User --> F114_Entity : creates/updates
    F114_Entity --> AuditLog : records
    F114_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F107 Open Restaurant Enrollment"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Open Restaurant Enrollment
    UI->>API: Request /api/v1/open_restaurant_enrollment
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F107 data
...
classDiagram
    ...
    User --> F107_Entity : creates/updates
    F107_Entity --> AuditLog : records
    F107_Entity --> NotificationLog : triggers
```

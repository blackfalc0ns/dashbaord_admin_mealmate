...
```mermaid
flowchart TD
    Start(("Start F116 Campaign Accounting"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Accounting
    UI->>API: Request /api/v1/campaign_accounting
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F116 data
...
classDiagram
    ...
    User --> F116_Entity : creates/updates
    F116_Entity --> AuditLog : records
    F116_Entity --> NotificationLog : triggers
```

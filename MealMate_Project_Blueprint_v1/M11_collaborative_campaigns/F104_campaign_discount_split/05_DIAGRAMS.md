...
```mermaid
flowchart TD
    Start(("Start F104 Campaign Discount Split"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Discount Split
    UI->>API: Request /api/v1/campaign_discount_split
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F104 data
...
classDiagram
    ...
    User --> F104_Entity : creates/updates
    F104_Entity --> AuditLog : records
    F104_Entity --> NotificationLog : triggers
```

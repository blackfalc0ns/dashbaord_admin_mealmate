...
```mermaid
flowchart TD
    Start(("Start F048 Second Delivery Attempt"))
    S1["فتح الطلب المسند."]
...
sequenceDiagram
    ...
    User->>UI: Open Second Delivery Attempt
    UI->>API: Request /api/v1/second_delivery_attempt
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F048 data
...
classDiagram
    ...
    User --> F048_Entity : creates/updates
    F048_Entity --> AuditLog : records
    F048_Entity --> NotificationLog : triggers
```

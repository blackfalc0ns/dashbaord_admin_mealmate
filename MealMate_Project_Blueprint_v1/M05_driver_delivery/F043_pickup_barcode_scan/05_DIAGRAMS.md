...
```mermaid
flowchart TD
    Start(("Start F043 Pickup Barcode Scan"))
    S1["فتح الطلب المسند."]
...
sequenceDiagram
    ...
    User->>UI: Open Pickup Barcode Scan
    UI->>API: Request /api/v1/pickup_barcode_scan
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F043 data
...
classDiagram
    ...
    User --> F043_Entity : creates/updates
    F043_Entity --> AuditLog : records
    F043_Entity --> NotificationLog : triggers
```

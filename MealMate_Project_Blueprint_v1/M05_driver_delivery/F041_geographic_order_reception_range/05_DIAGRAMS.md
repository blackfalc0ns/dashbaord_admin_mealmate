...
```mermaid
flowchart TD
    Start(("Start F041 Geographic Order Reception Range"))
    S1["فتح الطلب المسند."]
...
sequenceDiagram
    ...
    User->>UI: Open Geographic Order Reception Range
    UI->>API: Request /api/v1/geographic_order_reception_range
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F041 data
...
classDiagram
    ...
    User --> F041_Entity : creates/updates
    F041_Entity --> AuditLog : records
    F041_Entity --> NotificationLog : triggers
```

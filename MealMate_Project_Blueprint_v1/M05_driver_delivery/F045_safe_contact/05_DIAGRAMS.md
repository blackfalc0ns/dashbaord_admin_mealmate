...
sequenceDiagram
    ...
    User->>UI: Open Safe Contact
    UI->>API: Request /api/v1/safe_contact
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F045 data
...
classDiagram
    ...
    User --> F045_Entity : creates/updates
    F045_Entity --> AuditLog : records
    F045_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F050 Driver Rating"))
    S1["فتح الطلب المسند."]
...
sequenceDiagram
    ...
    User->>UI: Open Driver Rating
    UI->>API: Request /api/v1/driver_rating
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F050 data
...
classDiagram
    ...
    User --> F050_Entity : creates/updates
    F050_Entity --> AuditLog : records
    F050_Entity --> NotificationLog : triggers
```

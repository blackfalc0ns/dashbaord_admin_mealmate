...
```mermaid
flowchart TD
    Start(("Start F088 Restaurant Monthly Invoice"))
    S1["احتساب المستحق."]
    S2["إنشاء فاتورة/طلب."]
    S3["المراجعة."]
    S4["اعتماد الدفع."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Monthly Invoice
    UI->>API: Request /api/v1/restaurant_monthly_invoice
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F088 data
...
classDiagram
    ...
    User --> F088_Entity : creates/updates
    F088_Entity --> AuditLog : records
    F088_Entity --> NotificationLog : triggers
```

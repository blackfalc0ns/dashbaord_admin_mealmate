...
```mermaid
flowchart TD
    Start(("Start F089 Restaurant Partial Payment"))
    S1["احتساب المستحق."]
    S2["إنشاء فاتورة/طلب."]
    S3["المراجعة."]
    S4["اعتماد الدفع."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Partial Payment
    UI->>API: Request /api/v1/restaurant_partial_payment
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F089 data
...
classDiagram
    ...
    User --> F089_Entity : creates/updates
    F089_Entity --> AuditLog : records
    F089_Entity --> NotificationLog : triggers
```

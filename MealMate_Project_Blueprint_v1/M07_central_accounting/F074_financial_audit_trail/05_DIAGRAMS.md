...
```mermaid
flowchart TD
    Start(("Start F074 Financial Audit Trail"))
    S1["استقبال حدث مالي."]
    S2["التحقق من المصدر."]
...
sequenceDiagram
    ...
    User->>UI: Open Financial Audit Trail
    UI->>API: Request /api/v1/financial_audit_trail
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F074 data
...
classDiagram
    ...
    User --> F074_Entity : creates/updates
    F074_Entity --> AuditLog : records
    F074_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F073 Financial Reports"))
    S1["استقبال حدث مالي."]
    S2["التحقق من المصدر."]
...
sequenceDiagram
    ...
    User->>UI: Open Financial Reports
    UI->>API: Request /api/v1/financial_reports
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F073 data
...
classDiagram
    ...
    User --> F073_Entity : creates/updates
    F073_Entity --> AuditLog : records
    F073_Entity --> NotificationLog : triggers
```

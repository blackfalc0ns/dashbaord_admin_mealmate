...
```mermaid
flowchart TD
    Start(("Start F081 Cancellation Fees"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Cancellation Fees
    UI->>API: Request /api/v1/cancellation_fees
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F081 data
...
classDiagram
    ...
    User --> F081_Entity : creates/updates
    F081_Entity --> AuditLog : records
    F081_Entity --> NotificationLog : triggers
```

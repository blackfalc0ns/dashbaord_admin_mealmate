...
```mermaid
flowchart TD
    Start(("Start F077 Wallet Transactions Ledger"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Wallet Transactions Ledger
    UI->>API: Request /api/v1/wallet_transactions_ledger
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F077 data
...
classDiagram
    ...
    User --> F077_Entity : creates/updates
    F077_Entity --> AuditLog : records
    F077_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F082 Customer Withdrawal Request"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Customer Withdrawal Request
    UI->>API: Request /api/v1/customer_withdrawal_request
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F082 data
...
classDiagram
    ...
    User --> F082_Entity : creates/updates
    F082_Entity --> AuditLog : records
    F082_Entity --> NotificationLog : triggers
```

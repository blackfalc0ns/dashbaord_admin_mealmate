...
```mermaid
flowchart TD
    Start(("Start F084 Payments & Refund History"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Payments & Refund History
    UI->>API: Request /api/v1/payments_refund_history
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F084 data
...
classDiagram
    ...
    User --> F084_Entity : creates/updates
    F084_Entity --> AuditLog : records
    F084_Entity --> NotificationLog : triggers
```

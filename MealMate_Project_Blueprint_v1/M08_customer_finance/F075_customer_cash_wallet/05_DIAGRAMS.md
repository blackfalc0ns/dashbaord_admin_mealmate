...
```mermaid
flowchart TD
    Start(("Start F075 Customer Cash Wallet"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Customer Cash Wallet
    UI->>API: Request /api/v1/customer_cash_wallet
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F075 data
...
classDiagram
    ...
    User --> F075_Entity : creates/updates
    F075_Entity --> AuditLog : records
    F075_Entity --> NotificationLog : triggers
```

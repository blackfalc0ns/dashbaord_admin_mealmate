...
2. **تفصيل API Contract**
   - إضافة schemas لكل endpoint:
     - `GET /api/v1/subscription_payment_from_wallet`
     - `GET /api/v1/subscription_payment_from_wallet/{id}`
     - `POST /api/v1/subscription_payment_from_wallet`
     - `PUT /api/v1/subscription_payment_from_wallet/{id}`
     - `POST /api/v1/subscription_payment_from_wallet/{id}/actions/{action}`
...
3. **تفصيل صلاحيات Role/Scope**
   - ربط كل شاشة وكل endpoint بصلاحية مثل `F078.View`, `F078.Create`, `F078.Update`, `F078.Override`.

...
```mermaid
flowchart TD
    Start(("Start F127 Admin Reports"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Admin Reports
    UI->>API: Request /api/v1/admin_reports
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F127 data
...
classDiagram
    ...
    User --> F127_Entity : creates/updates
    F127_Entity --> AuditLog : records
    F127_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F123 Hold Cases Monitoring"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Hold Cases Monitoring
    UI->>API: Request /api/v1/hold_cases_monitoring
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F123 data
...
classDiagram
    ...
    User --> F123_Entity : creates/updates
    F123_Entity --> AuditLog : records
    F123_Entity --> NotificationLog : triggers
```

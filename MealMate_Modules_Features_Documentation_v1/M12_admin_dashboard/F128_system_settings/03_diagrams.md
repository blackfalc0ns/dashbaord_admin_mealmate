...
```mermaid
flowchart TD
    Start(("Start F128 System Settings"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open System Settings
    UI->>API: Request /api/v1/system_settings
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F128 data
...
classDiagram
    ...
    User --> F128_Entity : creates/updates
    F128_Entity --> AuditLog : records
    F128_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F124 Complaints Center"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Complaints Center
    UI->>API: Request /api/v1/complaints_center
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F124 data
...
classDiagram
    ...
    User --> F124_Entity : creates/updates
    F124_Entity --> AuditLog : records
    F124_Entity --> NotificationLog : triggers
```

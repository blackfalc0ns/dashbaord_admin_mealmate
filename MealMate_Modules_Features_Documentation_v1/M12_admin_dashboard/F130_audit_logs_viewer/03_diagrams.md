...
```mermaid
flowchart TD
    Start(("Start F130 Audit Logs Viewer"))
    S1["فتح الصفحة."]
    S2["تطبيق RBAC/Scope."]
...
sequenceDiagram
    ...
    User->>UI: Open Audit Logs Viewer
    UI->>API: Request /api/v1/audit_logs_viewer
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F130 data
...
classDiagram
    ...
    User --> F130_Entity : creates/updates
    F130_Entity --> AuditLog : records
    F130_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F106 Campaign Bundles"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Bundles
    UI->>API: Request /api/v1/campaign_bundles
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F106 data
...
classDiagram
    ...
    User --> F106_Entity : creates/updates
    F106_Entity --> AuditLog : records
    F106_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F103 Collaborative Campaign Creation"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Collaborative Campaign Creation
    UI->>API: Request /api/v1/collaborative_campaign_creation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F103 data
...
classDiagram
    ...
    User --> F103_Entity : creates/updates
    F103_Entity --> AuditLog : records
    F103_Entity --> NotificationLog : triggers
```

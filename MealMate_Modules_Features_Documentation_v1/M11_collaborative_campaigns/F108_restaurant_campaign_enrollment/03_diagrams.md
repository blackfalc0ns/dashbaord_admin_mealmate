...
```mermaid
flowchart TD
    Start(("Start F108 Restaurant Campaign Enrollment"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Campaign Enrollment
    UI->>API: Request /api/v1/restaurant_campaign_enrollment
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F108 data
...
classDiagram
    ...
    User --> F108_Entity : creates/updates
    F108_Entity --> AuditLog : records
    F108_Entity --> NotificationLog : triggers
```

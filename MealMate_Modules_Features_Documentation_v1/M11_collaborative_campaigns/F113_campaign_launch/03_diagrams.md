...
```mermaid
flowchart TD
    Start(("Start F113 Campaign Launch"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Launch
    UI->>API: Request /api/v1/campaign_launch
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F113 data
...
classDiagram
    ...
    User --> F113_Entity : creates/updates
    F113_Entity --> AuditLog : records
    F113_Entity --> NotificationLog : triggers
```

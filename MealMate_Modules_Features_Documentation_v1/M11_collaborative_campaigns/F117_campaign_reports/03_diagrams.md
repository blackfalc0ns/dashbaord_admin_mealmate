...
```mermaid
flowchart TD
    Start(("Start F117 Campaign Reports"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Reports
    UI->>API: Request /api/v1/campaign_reports
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F117 data
...
classDiagram
    ...
    User --> F117_Entity : creates/updates
    F117_Entity --> AuditLog : records
    F117_Entity --> NotificationLog : triggers
```

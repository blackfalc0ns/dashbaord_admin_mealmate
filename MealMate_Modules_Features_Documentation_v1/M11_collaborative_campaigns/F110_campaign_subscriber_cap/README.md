# F110 — سقف مشتركي الحملة

## Campaign Subscriber Cap
...
## الشرح المختصر
ميزة **سقف مشتركي الحملة** جزء من **الحملات التشاركية**. توثق هذه الصفحة السلوك التجاري والتشغيلي والتقني المطلوب، وتربط الواجهة والـBackend وقاعدة البيانات والصلاحيات والإشعارات والأثر المالي في مصدر واحد.
...
- **قاعدة خاصة:** السقف ≤ الطاقة المعتمدة.
- **قاعدة خاصة:** الإيقاف عند بلوغ السقف.
- **قاعدة خاصة:** الزيادة تحتاج موافقات.
...
### APIs
- `GET /api/v1/campaign_subscriber_cap`
- `GET /api/v1/campaign_subscriber_cap/{id}`
- `POST /api/v1/campaign_subscriber_cap`
- `PUT /api/v1/campaign_subscriber_cap/{id}`
- `POST /api/v1/campaign_subscriber_cap/{id}/actions/{action}`
### معايير API

...
```mermaid
flowchart TD
    Start(("Start F112 Campaign Restaurant Approval"))
    S1["إنشاء/فتح الحملة."]
...
sequenceDiagram
    ...
    User->>UI: Open Campaign Restaurant Approval
    UI->>API: Request /api/v1/campaign_restaurant_approval
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F112 data
...
classDiagram
    ...
    User --> F112_Entity : creates/updates
    F112_Entity --> AuditLog : records
    F112_Entity --> NotificationLog : triggers
```

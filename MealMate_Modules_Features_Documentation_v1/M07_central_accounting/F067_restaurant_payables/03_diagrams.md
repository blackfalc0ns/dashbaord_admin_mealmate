...
```mermaid
flowchart TD
    Start(("Start F067 Restaurant Payables"))
    S1["استقبال حدث مالي."]
    S2["التحقق من المصدر."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Payables
    UI->>API: Request /api/v1/restaurant_payables
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F067 data
...
classDiagram
    ...
    User --> F067_Entity : creates/updates
    F067_Entity --> AuditLog : records
    F067_Entity --> NotificationLog : triggers
```

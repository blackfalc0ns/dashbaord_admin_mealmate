...
```mermaid
flowchart TD
    Start(("Start F072 Financial Settlements"))
    S1["استقبال حدث مالي."]
    S2["التحقق من المصدر."]
...
sequenceDiagram
    ...
    User->>UI: Open Financial Settlements
    UI->>API: Request /api/v1/financial_settlements
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F072 data
...
classDiagram
    ...
    User --> F072_Entity : creates/updates
    F072_Entity --> AuditLog : records
    F072_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F065 Deferred Revenue"))
    S1["استقبال حدث مالي."]
    S2["التحقق من المصدر."]
...
sequenceDiagram
    ...
    User->>UI: Open Deferred Revenue
    UI->>API: Request /api/v1/deferred_revenue
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F065 data
...
classDiagram
    ...
    User --> F065_Entity : creates/updates
    F065_Entity --> AuditLog : records
    F065_Entity --> NotificationLog : triggers
```

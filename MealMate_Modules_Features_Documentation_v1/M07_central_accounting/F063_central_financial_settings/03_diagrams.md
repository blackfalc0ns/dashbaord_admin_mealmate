...
```mermaid
flowchart TD
    Start(("Start F063 Central Financial Settings"))
    S1["استقبال حدث مالي."]
    S2["التحقق من المصدر."]
...
sequenceDiagram
    ...
    User->>UI: Open Central Financial Settings
    UI->>API: Request /api/v1/central_financial_settings
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F063 data
...
classDiagram
    ...
    User --> F063_Entity : creates/updates
    F063_Entity --> AuditLog : records
    F063_Entity --> NotificationLog : triggers
```

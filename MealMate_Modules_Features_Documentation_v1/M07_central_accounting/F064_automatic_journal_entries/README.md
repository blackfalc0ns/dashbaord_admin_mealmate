# F064 — القيود المحاسبية التلقائية
...
## الشرح المختصر
ميزة **القيود المحاسبية التلقائية** جزء من **النظام المالي والمحاسبي المركزي**. توثق هذه الصفحة السلوك التجاري والتشغيلي والتقني المطلوب، وتربط الواجهة والـBackend وقاعدة البيانات والصلاحيات والإشعارات والأثر المالي في مصدر واحد.
...
- **قاعدة خاصة:** كل Business Event له قالب قيد.
- **قاعدة خاصة:** Debit = Credit قبل الترحيل.
- **قاعدة خاصة:** Idempotency تمنع التكرار.
...
### APIs
- `GET /api/v1/automatic_journal_entries`
- `GET /api/v1/automatic_journal_entries/{id}`
- `POST /api/v1/automatic_journal_entries`
- `PUT /api/v1/automatic_journal_entries/{id}`
- `POST /api/v1/automatic_journal_entries/{id}/actions/{action}`
### معايير API

...
```mermaid
flowchart TD
    Start(("Start F064 Automatic Journal Entries"))
    S1["استقبال حدث مالي."]
    S2["التحقق من المصدر."]
...
sequenceDiagram
    ...
    User->>UI: Open Automatic Journal Entries
    UI->>API: Request /api/v1/automatic_journal_entries
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F064 data
...
classDiagram
    ...
    User --> F064_Entity : creates/updates
    F064_Entity --> AuditLog : records
    F064_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F069 Payment Gateways"))
    S1["استقبال حدث مالي."]
    S2["التحقق من المصدر."]
...
sequenceDiagram
    ...
    User->>UI: Open Payment Gateways
    UI->>API: Request /api/v1/payment_gateways
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F069 data
...
classDiagram
    ...
    User --> F069_Entity : creates/updates
    F069_Entity --> AuditLog : records
    F069_Entity --> NotificationLog : triggers
```

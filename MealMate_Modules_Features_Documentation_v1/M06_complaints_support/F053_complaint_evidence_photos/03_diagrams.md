...
```mermaid
flowchart TD
    Start(("Start F053 Complaint Evidence Photos"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Complaint Evidence Photos
    UI->>API: Request /api/v1/complaint_evidence_photos
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F053 data
...
classDiagram
    ...
    User --> F053_Entity : creates/updates
    F053_Entity --> AuditLog : records
    F053_Entity --> NotificationLog : triggers
```

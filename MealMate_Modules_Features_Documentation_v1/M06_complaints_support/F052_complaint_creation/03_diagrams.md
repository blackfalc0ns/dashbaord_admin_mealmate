...
```mermaid
flowchart TD
    Start(("Start F052 Complaint Creation"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Complaint Creation
    UI->>API: Request /api/v1/complaint_creation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F052 data
...
classDiagram
    ...
    User --> F052_Entity : creates/updates
    F052_Entity --> AuditLog : records
    F052_Entity --> NotificationLog : triggers
```

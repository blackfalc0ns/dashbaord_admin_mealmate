...
```mermaid
flowchart TD
    Start(("Start F054 Complaint Classification"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Complaint Classification
    UI->>API: Request /api/v1/complaint_classification
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F054 data
...
classDiagram
    ...
    User --> F054_Entity : creates/updates
    F054_Entity --> AuditLog : records
    F054_Entity --> NotificationLog : triggers
```

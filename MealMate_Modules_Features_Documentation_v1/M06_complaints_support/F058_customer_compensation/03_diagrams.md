...
```mermaid
flowchart TD
    Start(("Start F058 Customer Compensation"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Customer Compensation
    UI->>API: Request /api/v1/customer_compensation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F058 data
...
classDiagram
    ...
    User --> F058_Entity : creates/updates
    F058_Entity --> AuditLog : records
    F058_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F055 Admin Complaint Review"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Admin Complaint Review
    UI->>API: Request /api/v1/admin_complaint_review
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F055 data
...
classDiagram
    ...
    User --> F055_Entity : creates/updates
    F055_Entity --> AuditLog : records
    F055_Entity --> NotificationLog : triggers
```

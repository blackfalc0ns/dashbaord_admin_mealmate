...
```mermaid
flowchart TD
    Start(("Start F057 Responsibility Decision"))
    S1["إنشاء الحالة."]
...
sequenceDiagram
    ...
    User->>UI: Open Responsibility Decision
    UI->>API: Request /api/v1/responsibility_decision
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F057 data
...
classDiagram
    ...
    User --> F057_Entity : creates/updates
    F057_Entity --> AuditLog : records
    F057_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F079 Subscription Cancellation"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Subscription Cancellation
    UI->>API: Request /api/v1/subscription_cancellation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F079 data
...
classDiagram
    ...
    User --> F079_Entity : creates/updates
    F079_Entity --> AuditLog : records
    F079_Entity --> NotificationLog : triggers
```

...
## الشرح المختصر
ميزة **المحفظة النقدية** جزء من **النظام المالي للعميل**. توثق هذه الصفحة السلوك التجاري والتشغيلي والتقني المطلوب، وتربط الواجهة والـBackend وقاعدة البيانات والصلاحيات والإشعارات والأثر المالي في مصدر واحد.
...
- **قاعدة خاصة:** يستقبل الاستردادات النقدية.
- **قاعدة خاصة:** غير سالب.
- **قاعدة خاصة:** كل حركة لها SourceReference.
...
### APIs
- `GET /api/v1/customer_cash_wallet`
- `GET /api/v1/customer_cash_wallet/{id}`
- `POST /api/v1/customer_cash_wallet`
- `PUT /api/v1/customer_cash_wallet/{id}`
- `POST /api/v1/customer_cash_wallet/{id}/actions/{action}`
### معايير API

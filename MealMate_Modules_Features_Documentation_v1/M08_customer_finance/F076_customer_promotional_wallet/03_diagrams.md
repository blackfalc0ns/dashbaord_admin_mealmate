...
```mermaid
flowchart TD
    Start(("Start F076 Customer Promotional Wallet"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Customer Promotional Wallet
    UI->>API: Request /api/v1/customer_promotional_wallet
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F076 data
...
classDiagram
    ...
    User --> F076_Entity : creates/updates
    F076_Entity --> AuditLog : records
    F076_Entity --> NotificationLog : triggers
```

...
2. **تفصيل API Contract**
   - إضافة schemas لكل endpoint:
     - `GET /api/v1/cancellation_fees`
     - `GET /api/v1/cancellation_fees/{id}`
     - `POST /api/v1/cancellation_fees`
     - `PUT /api/v1/cancellation_fees/{id}`
     - `POST /api/v1/cancellation_fees/{id}/actions/{action}`
...
3. **تفصيل صلاحيات Role/Scope**
   - ربط كل شاشة وكل endpoint بصلاحية مثل `F081.View`, `F081.Create`, `F081.Update`, `F081.Override`.

...
```mermaid
flowchart TD
    Start(("Start F080 Refund Calculation"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Refund Calculation
    UI->>API: Request /api/v1/refund_calculation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F080 data
...
classDiagram
    ...
    User --> F080_Entity : creates/updates
    F080_Entity --> AuditLog : records
    F080_Entity --> NotificationLog : triggers
```

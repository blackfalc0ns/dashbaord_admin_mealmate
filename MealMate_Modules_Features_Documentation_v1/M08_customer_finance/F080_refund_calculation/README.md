...
## الشرح المختصر
ميزة **حساب الاسترداد** جزء من **النظام المالي للعميل**. توثق هذه الصفحة السلوك التجاري والتشغيلي والتقني المطلوب، وتربط الواجهة والـBackend وقاعدة البيانات والصلاحيات والإشعارات والأثر المالي في مصدر واحد.
...
- **قاعدة خاصة:** remaining=max(total-used-buffer,0).
- **قاعدة خاصة:** day_value=paid/total_days.
- **قاعدة خاصة:** refund=eligible-cancellation_fee.
...
### APIs
- `GET /api/v1/refund_calculation`
- `GET /api/v1/refund_calculation/{id}`
- `POST /api/v1/refund_calculation`
- `PUT /api/v1/refund_calculation/{id}`
- `POST /api/v1/refund_calculation/{id}/actions/{action}`
### معايير API

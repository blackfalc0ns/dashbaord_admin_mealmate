...
```mermaid
flowchart TD
    Start(("Start F078 Subscription Payment from Wallet"))
    S1["عرض الرصيد."]
    S2["التحقق من الأهلية."]
    S3["إنشاء حركة Pending."]
    S4["المراجعة."]
    S5["التنفيذ."]
    S6["تحديث الرصيد والسجل."]
    Done(("Done"))
    Start --> S1
    S1 --> S2
    S2 --> S3
...
sequenceDiagram
    ...
    User->>UI: Open Subscription Payment from Wallet
    UI->>API: Request /api/v1/subscription_payment_from_wallet
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F078 data
...
classDiagram
    ...
    User --> F078_Entity : creates/updates
    F078_Entity --> AuditLog : records
    F078_Entity --> NotificationLog : triggers
```

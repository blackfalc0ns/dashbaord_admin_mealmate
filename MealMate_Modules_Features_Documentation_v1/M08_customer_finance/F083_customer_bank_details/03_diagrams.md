...
```mermaid
flowchart TD
    Start(("Start F083 Customer Bank Details"))
    S1["عرض الرصيد."]
...
sequenceDiagram
    ...
    User->>UI: Open Customer Bank Details
    UI->>API: Request /api/v1/customer_bank_details
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F083 data
...
classDiagram
    ...
    User --> F083_Entity : creates/updates
    F083_Entity --> AuditLog : records
    F083_Entity --> NotificationLog : triggers
```

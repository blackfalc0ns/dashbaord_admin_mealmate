...
```mermaid
flowchart TD
    Start(("Start F028 Menu & Meal Management"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Menu & Meal Management
    UI->>API: Request /api/v1/menu_meal_management
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F028 data
...
classDiagram
    ...
    User --> F028_Entity : creates/updates
    F028_Entity --> AuditLog : records
    F028_Entity --> NotificationLog : triggers
```

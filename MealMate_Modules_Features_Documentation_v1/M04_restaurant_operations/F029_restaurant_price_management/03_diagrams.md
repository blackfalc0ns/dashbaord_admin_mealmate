...
```mermaid
flowchart TD
    Start(("Start F029 Restaurant Price Management"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Price Management
    UI->>API: Request /api/v1/restaurant_price_management
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F029 data
...
classDiagram
    ...
    User --> F029_Entity : creates/updates
    F029_Entity --> AuditLog : records
    F029_Entity --> NotificationLog : triggers
```

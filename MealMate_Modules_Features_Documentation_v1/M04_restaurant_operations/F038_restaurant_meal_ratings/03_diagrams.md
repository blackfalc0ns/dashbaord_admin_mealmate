...
```mermaid
flowchart TD
    Start(("Start F038 Restaurant & Meal Ratings"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant & Meal Ratings
    UI->>API: Request /api/v1/restaurant_meal_ratings
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F038 data
...
classDiagram
    ...
    User --> F038_Entity : creates/updates
    F038_Entity --> AuditLog : records
    F038_Entity --> NotificationLog : triggers
```

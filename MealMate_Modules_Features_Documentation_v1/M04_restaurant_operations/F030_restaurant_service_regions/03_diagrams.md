...
```mermaid
flowchart TD
    Start(("Start F030 Restaurant Service Regions"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Service Regions
    UI->>API: Request /api/v1/restaurant_service_regions
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F030 data
...
classDiagram
    ...
    User --> F030_Entity : creates/updates
    F030_Entity --> AuditLog : records
    F030_Entity --> NotificationLog : triggers
```

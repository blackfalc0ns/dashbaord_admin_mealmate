...
```mermaid
flowchart TD
    Start(("Start F031 Daily Capacity Management"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Daily Capacity Management
    UI->>API: Request /api/v1/daily_capacity_management
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F031 data
...
classDiagram
    ...
    User --> F031_Entity : creates/updates
    F031_Entity --> AuditLog : records
    F031_Entity --> NotificationLog : triggers
```

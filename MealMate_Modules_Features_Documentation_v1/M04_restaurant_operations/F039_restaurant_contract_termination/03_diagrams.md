...
```mermaid
flowchart TD
    Start(("Start F039 Restaurant Contract Termination"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Contract Termination
    UI->>API: Request /api/v1/restaurant_contract_termination
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F039 data
...
classDiagram
    ...
    User --> F039_Entity : creates/updates
    F039_Entity --> AuditLog : records
    F039_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F034 Restaurant 72h Confirmation"))
    S1["فتح وظيفة المطعم."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant 72h Confirmation
    UI->>API: Request /api/v1/restaurant_72h_confirmation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F034 data
...
classDiagram
    ...
    User --> F034_Entity : creates/updates
    F034_Entity --> AuditLog : records
    F034_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F011 Restaurant Bundle Pricing"))
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Bundle Pricing
    UI->>API: Request /api/v1/restaurant_bundle_pricing
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F011 data
...
classDiagram
    ...
    User --> F011_Entity : creates/updates
    F011_Entity --> AuditLog : records
    F011_Entity --> NotificationLog : triggers
```

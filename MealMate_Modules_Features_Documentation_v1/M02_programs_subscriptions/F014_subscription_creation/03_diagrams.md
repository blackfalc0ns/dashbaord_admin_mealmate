...
```mermaid
flowchart TD
    Start(("Start F014 Subscription Creation"))
...
sequenceDiagram
    ...
    User->>UI: Open Subscription Creation
    UI->>API: Request /api/v1/subscription_creation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F014 data
...
classDiagram
    ...
    User --> F014_Entity : creates/updates
    F014_Entity --> AuditLog : records
    F014_Entity --> NotificationLog : triggers
```

...
sequenceDiagram
    ...
    User->>UI: Open Nutrition Programs Management
    UI->>API: Request /api/v1/nutrition_programs_management
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F009 data
...
classDiagram
    ...
    User --> F009_Entity : creates/updates
    F009_Entity --> AuditLog : records
    F009_Entity --> NotificationLog : triggers
```

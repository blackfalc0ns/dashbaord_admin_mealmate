...
```mermaid
flowchart TD
    Start(("Start F018 Subscription Renewal"))
...
sequenceDiagram
    ...
    User->>UI: Open Subscription Renewal
    UI->>API: Request /api/v1/subscription_renewal
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F018 data
...
classDiagram
    ...
    User --> F018_Entity : creates/updates
    F018_Entity --> AuditLog : records
    F018_Entity --> NotificationLog : triggers
```

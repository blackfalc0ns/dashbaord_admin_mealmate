...
```mermaid
flowchart TD
    Start(("Start F004 Influencer Registration & Account Linking"))
    S1["بدء الطلب."]
...
sequenceDiagram
    ...
    User->>UI: Open Influencer Registration & Account Linking
    UI->>API: Request /api/v1/influencer_registration_account_linking
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F004 data
...
classDiagram
    ...
    User --> F004_Entity : creates/updates
    F004_Entity --> AuditLog : records
    F004_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F001 Customer Registration, Login & Onboarding"))
    S1["بدء الطلب."]
...
sequenceDiagram
    ...
    User->>UI: Open Customer Registration, Login & Onboarding
    UI->>API: Request /api/v1/customer_registration_login_onboarding
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F001 data
...
classDiagram
    ...
    User --> F001_Entity : creates/updates
    F001_Entity --> AuditLog : records
    F001_Entity --> NotificationLog : triggers
```

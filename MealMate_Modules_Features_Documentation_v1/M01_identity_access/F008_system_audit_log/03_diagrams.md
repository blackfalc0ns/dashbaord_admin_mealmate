...
```mermaid
flowchart TD
    Start(("Start F008 System Audit Log"))
    S1["بدء الطلب."]
...
sequenceDiagram
    ...
    User->>UI: Open System Audit Log
    UI->>API: Request /api/v1/system_audit_log
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F008 data
...
classDiagram
    ...
    User --> F008_Entity : creates/updates
    F008_Entity --> AuditLog : records
    F008_Entity --> NotificationLog : triggers
```

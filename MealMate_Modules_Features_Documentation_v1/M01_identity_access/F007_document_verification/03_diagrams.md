...
```mermaid
flowchart TD
    Start(("Start F007 Document Verification"))
    S1["بدء الطلب."]
...
sequenceDiagram
    ...
    User->>UI: Open Document Verification
    UI->>API: Request /api/v1/document_verification
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F007 data
...
classDiagram
    ...
    User --> F007_Entity : creates/updates
    F007_Entity --> AuditLog : records
    F007_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F003 Driver Registration & Approval"))
    S1["بدء الطلب."]
...
sequenceDiagram
    ...
    User->>UI: Open Driver Registration & Approval
    UI->>API: Request /api/v1/driver_registration_approval
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F003 data
...
classDiagram
    ...
    User --> F003_Entity : creates/updates
    F003_Entity --> AuditLog : records
    F003_Entity --> NotificationLog : triggers
```

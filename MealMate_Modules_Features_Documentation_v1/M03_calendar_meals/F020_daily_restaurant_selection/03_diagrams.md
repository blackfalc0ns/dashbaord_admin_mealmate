...
```mermaid
flowchart TD
    Start(("Start F020 Daily Restaurant Selection"))
    S1["فتح التقويم."]
...
sequenceDiagram
    ...
    User->>UI: Open Daily Restaurant Selection
    UI->>API: Request /api/v1/daily_restaurant_selection
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F020 data
...
classDiagram
    ...
    User --> F020_Entity : creates/updates
    F020_Entity --> AuditLog : records
    F020_Entity --> NotificationLog : triggers
```

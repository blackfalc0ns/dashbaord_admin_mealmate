...
```mermaid
flowchart TD
    Start(("Start F021 Daily Meal Selection"))
    S1["فتح التقويم."]
...
sequenceDiagram
    ...
    User->>UI: Open Daily Meal Selection
    UI->>API: Request /api/v1/daily_meal_selection
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F021 data
...
classDiagram
    ...
    User --> F021_Entity : creates/updates
    F021_Entity --> AuditLog : records
    F021_Entity --> NotificationLog : triggers
```

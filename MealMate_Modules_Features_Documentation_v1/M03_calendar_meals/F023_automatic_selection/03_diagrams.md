...
```mermaid
flowchart TD
    Start(("Start F023 Automatic Selection"))
    S1["فتح التقويم."]
...
sequenceDiagram
    ...
    User->>UI: Open Automatic Selection
    UI->>API: Request /api/v1/automatic_selection
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F023 data
...
classDiagram
    ...
    User --> F023_Entity : creates/updates
    F023_Entity --> AuditLog : records
    F023_Entity --> NotificationLog : triggers
```

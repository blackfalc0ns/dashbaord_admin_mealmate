...
```mermaid
flowchart TD
    Start(("Start F027 Subscription Freeze"))
    S1["فتح التقويم."]
...
sequenceDiagram
    ...
    User->>UI: Open Subscription Freeze
    UI->>API: Request /api/v1/subscription_freeze
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F027 data
...
classDiagram
    ...
    User --> F027_Entity : creates/updates
    F027_Entity --> AuditLog : records
    F027_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F026 Meal Change Management"))
    S1["فتح التقويم."]
...
sequenceDiagram
    ...
    User->>UI: Open Meal Change Management
    UI->>API: Request /api/v1/meal_change_management
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F026 data
...
classDiagram
    ...
    User --> F026_Entity : creates/updates
    F026_Entity --> AuditLog : records
    F026_Entity --> NotificationLog : triggers
```

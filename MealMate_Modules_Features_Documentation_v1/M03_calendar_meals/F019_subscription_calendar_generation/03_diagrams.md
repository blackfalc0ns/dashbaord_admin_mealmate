...
```mermaid
flowchart TD
    Start(("Start F019 Subscription Calendar Generation"))
    S1["فتح التقويم."]
...
sequenceDiagram
    ...
    User->>UI: Open Subscription Calendar Generation
    UI->>API: Request /api/v1/subscription_calendar_generation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F019 data
...
classDiagram
    ...
    User --> F019_Entity : creates/updates
    F019_Entity --> AuditLog : records
    F019_Entity --> NotificationLog : triggers
```

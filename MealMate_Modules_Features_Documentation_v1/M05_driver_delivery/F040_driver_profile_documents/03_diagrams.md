...
```mermaid
flowchart TD
    Start(("Start F040 Driver Profile & Documents"))
    S1["فتح الطلب المسند."]
...
sequenceDiagram
    ...
    User->>UI: Open Driver Profile & Documents
    UI->>API: Request /api/v1/driver_profile_documents
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F040 data
...
classDiagram
    ...
    User --> F040_Entity : creates/updates
    F040_Entity --> AuditLog : records
    F040_Entity --> NotificationLog : triggers
```

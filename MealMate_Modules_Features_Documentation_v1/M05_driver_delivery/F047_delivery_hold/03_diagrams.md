...
```mermaid
flowchart TD
    Start(("Start F047 Delivery Hold"))
    S1["فتح الطلب المسند."]
...
sequenceDiagram
    ...
    User->>UI: Open Delivery Hold
    UI->>API: Request /api/v1/delivery_hold
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F047 data
...
classDiagram
    ...
    User --> F047_Entity : creates/updates
    F047_Entity --> AuditLog : records
    F047_Entity --> NotificationLog : triggers
```

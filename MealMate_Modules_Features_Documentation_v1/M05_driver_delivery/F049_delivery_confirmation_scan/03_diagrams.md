...
```mermaid
flowchart TD
    Start(("Start F049 Delivery Confirmation Scan"))
    S1["فتح الطلب المسند."]
...
sequenceDiagram
    ...
    User->>UI: Open Delivery Confirmation Scan
    UI->>API: Request /api/v1/delivery_confirmation_scan
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F049 data
...
classDiagram
    ...
    User --> F049_Entity : creates/updates
    F049_Entity --> AuditLog : records
    F049_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F044 Real-Time Delivery Tracking"))
    S1["فتح الطلب المسند."]
...
sequenceDiagram
    ...
    User->>UI: Open Real-Time Delivery Tracking
    UI->>API: Request /api/v1/real_time_delivery_tracking
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F044 data
...
classDiagram
    ...
    User --> F044_Entity : creates/updates
    F044_Entity --> AuditLog : records
    F044_Entity --> NotificationLog : triggers
```

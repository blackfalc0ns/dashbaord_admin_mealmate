...
```mermaid
flowchart TD
    Start(("Start F086 Restaurant Commission"))
    S1["احتساب المستحق."]
    S2["إنشاء فاتورة/طلب."]
    S3["المراجعة."]
    S4["اعتماد الدفع."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Commission
    UI->>API: Request /api/v1/restaurant_commission
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F086 data
...
classDiagram
    ...
    User --> F086_Entity : creates/updates
    F086_Entity --> AuditLog : records
    F086_Entity --> NotificationLog : triggers
```

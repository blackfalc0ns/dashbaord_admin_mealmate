...
```mermaid
flowchart TD
    Start(("Start F091 Transfer Receipt"))
    S1["احتساب المستحق."]
    S2["إنشاء فاتورة/طلب."]
    S3["المراجعة."]
    S4["اعتماد الدفع."]
...
sequenceDiagram
    ...
    User->>UI: Open Transfer Receipt
    UI->>API: Request /api/v1/transfer_receipt
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F091 data
...
classDiagram
    ...
    User --> F091_Entity : creates/updates
    F091_Entity --> AuditLog : records
    F091_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F087 Complaint Deductions"))
    S1["احتساب المستحق."]
    S2["إنشاء فاتورة/طلب."]
    S3["المراجعة."]
    S4["اعتماد الدفع."]
...
sequenceDiagram
    ...
    User->>UI: Open Complaint Deductions
    UI->>API: Request /api/v1/complaint_deductions
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F087 data
...
classDiagram
    ...
    User --> F087_Entity : creates/updates
    F087_Entity --> AuditLog : records
    F087_Entity --> NotificationLog : triggers
```

...
```mermaid
flowchart TD
    Start(("Start F092 Restaurant Account Statement"))
    S1["احتساب المستحق."]
    S2["إنشاء فاتورة/طلب."]
    S3["المراجعة."]
    S4["اعتماد الدفع."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Account Statement
    UI->>API: Request /api/v1/restaurant_account_statement
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F092 data
...
classDiagram
    ...
    User --> F092_Entity : creates/updates
    F092_Entity --> AuditLog : records
    F092_Entity --> NotificationLog : triggers
```

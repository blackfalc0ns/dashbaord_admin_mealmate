...
```mermaid
flowchart TD
    Start(("Start F093 Restaurant Financial Reports"))
    S1["احتساب المستحق."]
    S2["إنشاء فاتورة/طلب."]
    S3["المراجعة."]
    S4["اعتماد الدفع."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Financial Reports
    UI->>API: Request /api/v1/restaurant_financial_reports
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F093 data
...
classDiagram
    ...
    User --> F093_Entity : creates/updates
    F093_Entity --> AuditLog : records
    F093_Entity --> NotificationLog : triggers
```

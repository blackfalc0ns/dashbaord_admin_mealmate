...
```mermaid
flowchart TD
    Start(("Start F090 Restaurant Payout"))
    S1["احتساب المستحق."]
    S2["إنشاء فاتورة/طلب."]
    S3["المراجعة."]
    S4["اعتماد الدفع."]
...
sequenceDiagram
    ...
    User->>UI: Open Restaurant Payout
    UI->>API: Request /api/v1/restaurant_payout
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F090 data
...
classDiagram
    ...
    User --> F090_Entity : creates/updates
    F090_Entity --> AuditLog : records
    F090_Entity --> NotificationLog : triggers
```

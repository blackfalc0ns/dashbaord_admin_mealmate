...
```mermaid
flowchart TD
    Start(("Start F096 Influencer Commission Calculation"))
    S1["ربط المؤثر/الكود."]
...
sequenceDiagram
    ...
    User->>UI: Open Influencer Commission Calculation
    UI->>API: Request /api/v1/influencer_commission_calculation
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F096 data
...
classDiagram
    ...
    User --> F096_Entity : creates/updates
    F096_Entity --> AuditLog : records
    F096_Entity --> NotificationLog : triggers
```

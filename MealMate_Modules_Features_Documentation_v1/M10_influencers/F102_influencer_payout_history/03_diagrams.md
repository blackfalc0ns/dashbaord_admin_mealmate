...
```mermaid
flowchart TD
    Start(("Start F102 Influencer Payout History"))
    S1["ربط المؤثر/الكود."]
...
sequenceDiagram
    ...
    User->>UI: Open Influencer Payout History
    UI->>API: Request /api/v1/influencer_payout_history
    API->>Auth: Validate token, permission, scope
    Auth-->>API: Authorized
    API->>Domain: Execute business rules
    Domain->>DB: Read/write F102 data
...
classDiagram
    ...
    User --> F102_Entity : creates/updates
    F102_Entity --> AuditLog : records
    F102_Entity --> NotificationLog : triggers
```
